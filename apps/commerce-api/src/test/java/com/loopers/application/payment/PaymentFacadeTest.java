...
@DisplayName("PaymentFacade 단위 테스트 (V2 Resilience4j)")
class PaymentFacadeTest {
    ...
    class ProcessPayment {
        ...
        void processesPaymentWithSeparateTransactions() {
            // given
            Long memberId = 1L;
            Long orderId = 100L;
            ...
            // when
            PaymentInfo result = paymentFacade.processPayment(memberId, orderId, "SAMSUNG", "1234");

            // then
            assertAll(
                    () -> assertThat(result.status()).isEqualTo(PaymentStatus.PENDING),
                    () -> assertThat(result.orderId()).isEqualTo(orderId),
                    () -> verify(paymentService).initiatePayment(memberId, orderId, 50000, "SAMSUNG", "1234"),
                    () -> verify(paymentGateway).requestPayment(eq(memberId), any(Payment.class)),
        ...
        @Test
        @DisplayName("PaymentGateway가 예외를 던져도 Facade는 예외를 전파한다.")
    ...
    class HandleCallback {
        ...
        void completesOrderOnSuccessCallback() {
            ...
            // when
            paymentFacade.handleCallback(orderId, txId, "SUCCESS");
        ...
        void failsOrderAndRestoresStockOnFailedCallback() {
            ...
            // when
            paymentFacade.handleCallback(orderId, txId, "LIMIT_EXCEEDED");

            // then
            verify(paymentService).syncPaymentResult(orderId, txId, "LIMIT_EXCEEDED");
    ...
    class VerifyPayment {
        ...
        void completesOrderWhenPgSuccess() {
            ...
            // when
            paymentFacade.verifyPayment(memberId, orderId);
        ...
        void marksRequestedAsFailedWhenPgHasNoResult() {
            ...
            // when
            PaymentInfo result = paymentFacade.verifyPayment(memberId, orderId);
