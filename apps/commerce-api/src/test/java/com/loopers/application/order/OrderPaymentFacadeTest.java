...
class OrderPaymentFacadeTest {
    ...
    class CreateOrder {
        ...
        @Test
        @DisplayName("주문 생성 후 PG 결제가 호출된다.")
        void createsOrderAndProcessesPayment() {
            ...
            // updatePaymentStatus 이후 조회 시 PENDING 상태 (accepted=true이므로)
            Payment pendingPayment = Payment.create(memberId, 1L, 100000, cardType, cardNo);
            ReflectionTestUtils.setField(pendingPayment, "id", 1L);
            ReflectionTestUtils.setField(pendingPayment, "status", PaymentStatus.PENDING);
            ...
            assertAll(
                    ...
                    () -> verify(paymentService, times(1)).updatePaymentStatus(1L,
                            new PaymentGatewayResponse("tx-123", true, false, null)),
                    () -> verify(orderCompensationService, never()).compensateFailedOrder(anyLong())
            );
        }

        @Test
        @DisplayName("전액 할인(결제 금액 0) 시 PG 결제 없이 바로 PAID 처리된다.")
        void completesOrderWithoutPayment_whenPaymentAmountIsZero() {
            // given
            Long memberId = 1L;
            List<OrderItemRequest> itemRequests = List.of(
        ...
        void compensatesOrder_whenPaymentFails() {
            ...
            // updatePaymentStatus 이후 조회 시 FAILED 상태
            ...
            // then
            assertAll(
                    () -> verify(paymentService, times(1)).updatePaymentStatus(1L, pgResponse),
