...
| Domain (Unit) | `MemberCouponTest` | 생성, use, isAvailable |
...
| `CouponUsageConcurrencyTest` | 동일 쿠폰 동시 사용 → 1번만 성공 | 낙관적 락 (@Version) |
...
- **선택 전략**: 낙관적 락 (`@Version`)
- **선택 이유**:
  - 쿠폰 사용은 동일 쿠폰을 여러 기기에서 동시에 사용하는 경합 빈도가 극히 낮음
  - fail-fast 동작이 적합: 충돌 시 `OptimisticLockingFailureException` 발생 → 트랜잭션 전체 롤백
  - DB 레벨 행 잠금을 점유하지 않아 대기 없이 빠르게 실패
  - `OrderFacade.createOrder()`의 `@Transactional` 범위에서 실행되므로, 낙관적 락 실패 시 재고 차감도 함께 롤백
- **변경 파일**:
  - `MemberCoupon` — `@Version @Column(name = "version") private Long version` 필드 추가
  - `CouponService.useCoupon()` — Javadoc으로 트랜잭션 필수 조건 명시 (Facade 트랜잭션에서 실행)
- 동시성 테스트(`CouponUsageConcurrencyTest`): `TransactionTemplate`으로 트랜잭션 감싸서 5개 스레드가 동시에 같은 MemberCoupon 사용 시도 → 1개 성공, 4개 실패, 최종 상태 USED 검증 통과
...
`OrderFacade.createOrder()`의 단일 `@Transactional` 내에서 두 가지 락 전략을 혼합 사용:

| 대상 | 전략 | 이유 |
|------|------|------|
| `ProductOption` (재고) | 비관적 락 | 경합 빈도 높음, 초과 판매 방지 필수, 순차 대기 허용 |
| `MemberCoupon` (쿠폰 사용) | 낙관적 락 | 경합 빈도 극히 낮음, fail-fast 적합, DB 락 미점유 |
| `Point` (포인트 잔액) | 비관적 락 | 회원당 1 row라 다른 회원에 영향 없음, 잔액 충분 시 순차 성공 허용 |

### 트랜잭션 롤백 보장

낙관적 락 실패(`OptimisticLockingFailureException`) 시:
1. Spring이 `@Transactional`에 의해 트랜잭션 전체를 롤백
2. 이미 차감된 재고(`ProductOption.stockQuantity`)도 롤백되어 원복
3. 생성된 주문(`Order`)도 롤백되어 삭제

→ 비관적 락(재고)과 낙관적 락(쿠폰)을 한 트랜잭션에서 혼합해도 원자성이 보장됨

---

## 낙관적 락(Optimistic Lock) 예외 처리 설계 — MemberCoupon 동시 사용

### 배경
- `MemberCoupon`에 `@Version`을 적용하여 낙관적 락으로 동시 쿠폰 사용을 방지
- 동시 주문 시 `OptimisticLockingFailureException`이 발생하는데, 이를 어떻게 처리할지 결정 필요

### 검토한 방안

#### 방안 1: `saveAndFlush()` + Service 내부 catch
- `useCoupon()`에서 `saveAndFlush()`로 즉시 SQL 실행 → `OptimisticLockingFailureException`을 Service에서 잡아 `CoreException`으로 변환
- **장점**: "쿠폰이 이미 사용되었습니다" 같은 명확한 비즈니스 메시지 반환 가능
- **단점**:
  - `flush()`는 해당 엔티티만이 아니라 **영속성 컨텍스트 전체를 flush** → 의도하지 않은 다른 엔티티의 SQL도 함께 실행됨
  - 다른 엔티티에도 `@Version`이 있다면 그 충돌도 이 시점에 함께 발생 → 어떤 엔티티 충돌인지 구분 다시 어려워짐
  - flush로 UPDATE가 먼저 나가면 해당 row에 DB 레벨 lock이 잡히고, 트랜잭션 끝까지 유지 → **락 보유 시간 증가**

#### 방안 2: 비관적 락으로 전환
- `findByIdWithLock()` (SELECT FOR UPDATE) → 상태 검증 → 상태 변경
- **장점**: 충돌 자체를 차단, 항상 비즈니스 예외만 발생
- **단점**: 충돌 빈도가 극히 낮은 시나리오에서 매번 행 잠금은 과도한 오버헤드

#### 방안 3: `save()` + GlobalExceptionHandler (채택)
- `useCoupon()`에서는 `isAvailable()` 비즈니스 검증만 수행, `save()`로 쓰기 지연 유지
- `@Version` 충돌은 트랜잭션 커밋 시 JPA가 자동으로 `OptimisticLockingFailureException`을 throw
- `ApiControllerAdvice`에서 해당 예외를 잡아 409 Conflict 응답
- **장점**: flush 부작용 없음, 비관적 락 오버헤드 없음, 기존 구조 변경 없음
- **단점**: JPA 내부 메시지이므로 "쿠폰 충돌"이라는 구체적 맥락은 알 수 없음

### 결정: 2단계 예외 처리 (방안 3)

**선택 근거**:
- 쿠폰 사용은 AVAILABLE → USED 단방향 1회성 전환이므로 재시도가 무의미
- 같은 쿠폰으로 동시 주문하는 충돌 빈도가 극히 낮아 비관적 락은 과도
- `flush()` 부작용(전체 영속성 컨텍스트 flush, 락 보유 시간 증가)을 피하기 위해 `save()` 유지

**예외 처리 흐름**:

| 단계 | 시점 | 처리 위치 | 메시지 |
|------|------|----------|--------|
| **1차** (대부분) | `useCoupon()` 내 `isAvailable()` | Service | "쿠폰이 이미 사용되었습니다" |
| **2차** (극소수) | 트랜잭션 커밋 시 `@Version` 충돌 | GlobalExceptionHandler | "동시 요청이 감지되었습니다" |

### Domain Service @Transactional(REQUIRED) 제거

**배경**: `CouponService.useCoupon()`과 `ProductService.deductStock()`에 `@Transactional`(기본 REQUIRED)이 있었으나, 둘 다 `OrderFacade.createOrder()`의 트랜잭션 안에서만 호출됨. REQUIRED는 기존 트랜잭션에 참여하므로 독립 트랜잭션이 아니며, 이로 인해:
- `save()` 시점에 SQL이 나가지 않고 Facade 트랜잭션 커밋 시 flush됨
- Service 내부에서 `OptimisticLockingFailureException`을 잡을 수 없음
...
- `04-erd.md` — PRODUCT_LIKE 테이블에 `(member_id, product_id)` UNIQUE 제약 추가, MEMBER_COUPON에 version 컬럼 반영
...
|:----:|------------|----------|---------|------|
| 1 | `Coupon`, `MemberCoupon` | 쿠폰 유효성 검증 | 락 없음 (읽기 전용) | 할인 금액 계산만 수행. TOCTOU 갭은 4단계 `@Version`에서 방어 |
...
| 4 | `MemberCoupon` | `status` (AVAILABLE → USED) | **낙관적 락** (`@Version`) | 충돌 시 `OptimisticLockingFailureException` → 트랜잭션 전체 롤백 (재고 차감 포함 원복) |
...
|------|----------|--------|-------------|
| `ObjectOptimisticLockingFailureException` | 409 CONFLICT | "동시 요청으로 처리에 실패했습니다. 다시 시도해주세요." | `MemberCoupon` `@Version` 충돌 (동일 쿠폰 동시 사용) |
...
- `MemberCoupon.use()`의 에러 메시지를 "이미 사용된 쿠폰입니다." → "사용할 수 없는 쿠폰입니다."로 변경 (USED/EXPIRED/DELETED 등 AVAILABLE이 아닌 모든 상태를 포괄)

**변경 파일**
- `CouponService.useCoupon()` — Coupon 조회 및 `coupon.isValid()` 유효기간 검증 추가
- `MemberCoupon.use()` — 에러 메시지 변경 ("사용할 수 없는 쿠폰입니다.")
- `CouponServiceTest` — 만료 쿠폰 사용 시도 테스트 추가, 성공 테스트에 couponRepository mock 추가
- `MemberCouponTest` — 에러 메시지 검증 수정