# 커밋했는데 이벤트가 사라졌다! — @TransactionalEventListener의 5가지 함정을 테스트로 증명하기

## TL;DR

`@TransactionalEventListener(AFTER_COMMIT)`은 비동기가 아니다.
같은 스레드에서 동기 실행되고, 리스너가 죽으면 클라이언트는 500을 받으며, 이벤트는 재시도 없이 유실된다.
5가지 함정을 테스트로 재현하고, Outbox Pattern + Consumer 멱등성으로 하나씩 해결해 나간 과정을 정리했다.
결론: 모든 이벤트에 Outbox가 필요한 건 아니다 — **"유실되면 얼마나 치명적인가"**가 전략 선택의 기준이다.
...
이 문제를 해결할 방법은 아래와 같이 세 가지가 있을 것이다.
...
결국, 이벤트 기반 아키텍처에서 가장 중요한 질문은 "어떻게 보낼 것인가"가 아니라 **"잃어버리면 어떻게 되는가"**였다.
